...
# /discord-sessions:configure — Discord Channel Setup
...
3. **What next** — end with a concrete next step based on state:
   - No token → *"Run `/discord-sessions:configure <token>` with your bot token from
     the Developer Portal → Bot → Reset Token."*
   - Token set, policy is pairing, nobody allowed → *"DM your bot on
     Discord. It replies with a code; approve with `/discord-sessions:access pair
     <code>`."*
...
3. **If yes and policy is still `pairing`** → *"Good. Let's lock it down so
   ...
   `/discord-sessions:access policy allowlist`. Do this proactively — don't wait to
   be asked.
4. **If no, people are missing** → *"Have them DM the bot; you'll approve
   each with `/discord-sessions:access pair <code>`. Run this skill again once
   ...
   right-click them → Copy User ID, then `/discord-sessions:access allow <id>`."*
...
- `access.json` is re-read on every inbound message — policy changes via
  `/discord-sessions:access` take effect immediately, no restart.
