...
- **Always-on watcher (optional).** A tiny autostart process keeps the bot online with zero sessions open: message a channel and a hidden session wakes for it (resuming its conversation), then shuts down after 30 idle minutes. A session waiting on an Allow/Deny click is never killed either, so the buttons still work when you come back to them hours later. A session that is still working is never killed: the watcher watches its transcript, not just the chat. If a turn dies on an API error (529 and friends), the watcher says so in the channel and resumes the work by itself, up to 3 times an hour. Terminal sessions always take priority. Windows tested; Linux/macOS support shipped via `install-watcher.sh` (systemd/launchd), not yet field-tested.
- **It tells you when you are logged out.** Claude Code's login expires now and then and `/login` needs a real terminal. The watcher notices and says so in the fallback channel instead of letting every session die on an auth error.
- **Buttons instead of commands.** Every channel gets a pinned panel: Plan, Continue, Stop, Restart, Show terminal, Hide terminal, Status, Usage. Questions come as a dropdown with an Autre entry that opens a free text field, so an answer never has to be typed twice.
- **Reachable from the Claude app.** An always-on concierge session (never reaped) is what Remote Control needs to exist on the machine: message it from the app and it wakes the real project session. `wake.mjs <name|path>` does the same from any terminal.
- **Several machines, one bot.** Run the watcher on two PCs and they sort out who wakes what through a `#claude-watchers` channel: the machine that has the session on disk takes it, and only one machine answers each command. Pin channels by hand in `watcher.json` if you want.
- **Watcher slash commands.** `/help`, `/sessions`, `/status`, `/logs`, `/usage` (plan limits and equivalent API cost per model, limits also shown permanently in the bot's status), `/skill` (searchable skill runner), `/kill` (emergency stop, nothing lost), `/killall`, `/restart`, `/open`/`/hide` (show a background session's live terminal on the PC), `/model` (per-channel model switch, and a channel keeps its own model even when you switch another one), `/rename-bot` (bot nickname), `/update`.
